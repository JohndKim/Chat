...
class Server(models.Model):
    name = models.CharField(max_length=100)
    # one to many (one owner can have many servers but one server can have one owner)
    owner = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="server_owner")
    # one to many (onw category can have many servers but one server can have one category)
    category = models.ForeignKey(Category, on_delete=models.CASCADE, related_name="server_category")
    description = models.CharField(max_length=250, blank=True, null=True)
    # many to many (one server can have many members and one member can have many servers)
...
class Channel(models.Model):
    ...
    # DATABASE SAVES THE PATH AND NAME OF THE IMAGES NOT THE ACTUAL IMAGES
    ...
    # saves (insert) data to db in lowercase always
    def save(self, *args, **kwargs):
